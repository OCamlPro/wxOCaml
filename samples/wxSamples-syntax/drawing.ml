...
let myCanvas_DrawTestBrushes frame (dc : wxDC) =
  ...
  dc->drawRectangle !x !y _WIDTH _HEIGHT;
  ...
  y := !y + _HEIGHT;
  dc->setBrush (wxBrushBitmap app.gs_bmpMask);
  dc->drawRectangle !x !y _WIDTH _HEIGHT;
  dc->drawText "Stipple mono" (!x + 10) (!y + 10);

  y := !y + _HEIGHT;
  dc->setBrush (wxBrushBitmap app.gs_bmpNoMask);
  dc->drawRectangle !x !y _WIDTH _HEIGHT;
...
let myCanvas_DrawTestLines frame (x, y, width, dc) =
  ...
  dc->drawText (Printf.sprintf "Testing lines of width %d" width)
    (x + 10) (y - 10);
...
module mdc -> WxMemoryDC in
  ...
  begin
    ...
    begin
      ...
      let (bool, c) = mdc->getPixel 11 11 in
      if bool then
        begin
          mdc->setBrush
            (wxBrush (wxColour 128 128 0 wxALPHA_OPAQUE) wxBRUSHSTYLE_SOLID);
          ignore_bool (mdc->floodFill 11 11 c wxFLOOD_SURFACE);
        end
    end;
  ...
  dc->drawBitmap (m_std_icon_bitmap) (!x + 5) ( 15) ( true);
  x += (rectSize + 10);
  dc->drawRectangle (!x) ( 10) ( rectSize) ( rectSize);
  ...
  if WxBitmap.isOk canvas.m_smile_bmp then
    dc->drawBitmap (canvas.m_smile_bmp) ( !x + rectSize - 20) ( rectSize - 10) ( true);
  ...
  dc->drawBitmap ( bitmap) ( 50) ( 170 ) false;
  ...
  memdc->setBackground  yellowBrush ;
  WxDC.clear memdc;
  ...
  memdc->drawRectangle (0) ( 0) ( totalWidth) ( totalHeight);
