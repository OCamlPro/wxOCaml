...
#define WXC_USE_TYPED_INTERFACE
typedef [abstract, ptr] void *voidptr;

/* Types that can be 'untyped' or C++ typed */
#ifdef WXC_USE_TYPED_INTERFACE
# define TClass(tp)     tp
# define TBool          boolean
# define TClosureFun    voidptr
#else
...
/* strings */
#define TString           [in, string] TChar*
#define TStringOut        [string] TChar*
...
/* structures */
#define TPoint(x,y)       [in] int x,  [in] int y
#define TPointOut(x,y)    [out] int* x, [out] int* y
#define TVector(x,y)      [in] int x,  [in] int y
#define TVectorOut(x,y)   [out] int* x, [out] int* y
#define TSize(w,h)        [in] int w,  [in] int h
#define TSizeOut(w,h)     [out] int* w, [out] int* h
#define TRect(x,y,w,h)    [in] int x,  [in] int y,  [in] int w,  [in] int h
#define TRectOut(x,y,w,h) [out] int* x, [out] int* y, [out] int* w, [out] int* h
#define TColorRGB(r,g,b)  TUInt8 r, TUInt8 g, TUInt8 b

/* arrays */
#define TArrayLen               int
#define TArrayIntOut            [out] int*
#define TArrayIntPtrOut         [out] int**
