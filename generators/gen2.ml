...
let new_class cl_name cl_parents cl_dirname =
  ...
  let cl_impl_mli = maybe_file cl_dirname "impl.mli" in
  let cl_impl_ml = maybe_file cl_dirname "impl.ml" in
  let cl_inher_ml = maybe_file cl_dirname "inher.ml" in
  let cl_inher_mli = maybe_file cl_dirname "inher.mli" in
  let cl_create =
    let mli_file = Filename.concat cl_dirname "wxnew.mli" in
...
let _ =
  ...
  List.iter (fun cl ->
    ...
    (match cl.cl_create with
     None -> ()
     | Some mli ->
       Printf.fprintf oc "  val wxnew : %s -> t\n" mli
    );
...
*)
  ...
  List.iter (fun cl ->
    (match cl.cl_create with
       None -> ()
     | Some mli ->
       Printf.fprintf oc "  let %s = %s.wxnew\n" cl.cl_name cl.cl_uname
    );
