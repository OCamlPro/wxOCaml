...
external resume : wxProgressDialog -> unit
	= "camlidl_wxc_wxProgressDialog_Resume"
...
external connect : wxProgressDialog -> int -> ('a WxEVT.t) -> ('a -> unit) -> unit
...
let wxnew title message max parent style =
  let title = WxString.createUTF8 title in
  let message = WxString.createUTF8 message in
  let wxres = wxnew title message max parent style  in
  WxString.delete title;
  WxString.delete message;
  wxres

let updateWithMessage obj _value message =
  let message = WxString.createUTF8 message in
  let wxres = updateWithMessage obj _value message  in
  WxString.delete message;
  wxres
...
external null_object : unit -> wxProgressDialog
