...
/* TODO
  With multiple inheritance, "this" may not be at the same position
when casting objects. We propose the following scheme: each Abstract_val
takes a runtime representation of the class (an int). For each method,
we know what type is expected, so, when we receive a value, we call
a C++ cast function from the received type to the expected type, a big
switch() that contains only the cast allowed by the class hierarchy.

We also need to find a way to call ancestors functions. For that, for
each virtual method that we redefine, we can generate a stub method
only calling the ancestor function.

We need to be more careful with C++ types, especially "const", as the
compiler will silently redefine a method instead of overriding the
virtual method if the types don't match exactly.
...
wxOCamlWizardPage::wxOCamlWizardPage(value callbacks_v, value state_v, 
		  wxWizard* parent_c, wxBitmap& bitmap_c):
    wxWizardPage(parent_c, bitmap_c),
    wxOCamlObject(WXCLASS_wxWizardPage, callbacks_v, state_v,
		  (wxWizardPage*) this) 
  {     
    //    fprintf(stderr, "self: %lX\n", this);
  }


wxWizardPage* wxOCamlWizardPage::GetPrev() const
  {
    //    fprintf(stderr, "x\n");
    value ret_v = caml_callback2(Field(m_callbacks_v, 0), 
				 m_this_v, 
				 m_state_v);
    return (wxWizardPage*)AbstractOption_val(WXCLASS_wxWizardPage, ret_v);
  }

  wxWizardPage* wxOCamlWizardPage::GetNext() const
  {
    //    fprintf(stderr, "y\n");
  ...
  /* TODO : a method to call parent methods from overriden methods 
  wxBitmap wxOCamlWizardPage::super_GetBitmap()
  {
    return wxWizardPage::GetBitmap();
  }
  */

  wxBitmap wxOCamlWizardPage::GetBitmap() const
  {
    wxBitmap* ret_c;
    value callback_v = Field(m_callbacks_v, 2);
    if( callback_v == Val_unit ){
      return wxWizardPage::GetBitmap();
    } else {
      value ret_v = caml_callback2(Field(callback_v, 0), 
				   m_this_v, 
				   m_state_v);
      //      fprintf(stderr, "ret_v = %lX\n", ret_v);
      ret_c = (wxBitmap*)Abstract_val(WXCLASS_wxBitmap, ret_v);
      return *ret_c;
    }
  }

  bool wxOCamlWizardPage::TransferDataFromWindow()
  {
    bool ret_c;
    value callback_v = Field(m_callbacks_v, 3);
    if( callback_v == Val_unit ){
      ...
      value ret_v = caml_callback2(Field(callback_v, 0), 
				   m_this_v, 
				   m_state_v);
      return Int_val(ret_v);
    }
  }

  bool wxOCamlWizardPage::TransferDataToWindow()
  {
    bool ret_c;
    /*
    fprintf(stderr, "getting m_callbacks_v to %lX\n", m_callbacks_v);
    for(int i = 0; i < 5; i++){
      fprintf(stderr, "Field(_, %d) = %lX\n", i, Field(m_callbacks_v, i));
    }
    */

    // fprintf(stderr, "TransferDataToWindow: %lX\n", this);
    value callback_v = Field(m_callbacks_v, 4);
    if( callback_v == Val_unit ){
  ...
  bool wxOCamlWizardPage::Validate()
  {
    bool ret_c;
    value callback_v = Field(m_callbacks_v, 5);
    if( callback_v == Val_unit ){
...
value wxOCamlWizardPage_Create_c(value callback_v, value state_v, value parent_v, value bitmap_v)
{
  CAMLparam0();
  CAMLlocal1(ret_v);    
  //  fprintf(stderr, "init m_callbacks_v to %lX\n", callback_v);

  wxWizard* parent_c = (wxWizard*)AbstractOption_val(WXCLASS_wxWizard, parent_v);
  wxBitmap* bitmap_c = (wxBitmap*)Abstract_val(WXCLASS_wxBitmap, bitmap_v);
  wxOCamlWizardPage * ret_c = new wxOCamlWizardPage(callback_v, state_v, parent_c, *bitmap_c);
