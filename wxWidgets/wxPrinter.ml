...
external getPrintDialogData : wxPrinter -> wxPrintDialogData -> unit
	= "camlidl_wxc_wxPrinter_GetPrintDialogData"

external getLastError : wxPrinter -> int
	= "camlidl_wxc_wxPrinter_GetLastError"
...
external getAbort : wxPrinter -> bool
	= "camlidl_wxc_wxPrinter_GetAbort"

external delete : wxPrinter -> unit
	= "camlidl_wxc_wxPrinter_Delete"

external createAbortWindow : wxPrinter -> wxWindow -> wxPrintout -> wxWindow
	= "camlidl_wxc_wxPrinter_CreateAbortWindow"

let reportError _obj parent printout message =
  let message = WxString.createUTF8 message in
  let wxres = reportError _obj parent printout message  in
  WxString.delete message;
  wxres

external null_object : unit -> wxPrinter
